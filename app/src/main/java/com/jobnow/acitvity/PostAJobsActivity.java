...
import com.jobnow.adapter.CategoryIndustryAdapter;
import com.jobnow.adapter.EmploymentTypeAdapter;
import com.jobnow.adapter.LevelAdapter;
import com.jobnow.adapter.ListExperienceAdapter;
import com.jobnow.adapter.LocationAdapter;
import com.jobnow.common.APICommon;
import com.jobnow.common.MultiSpinner;
import com.jobnow.config.Config;
import com.jobnow.controller.JobController;
import com.jobnow.models.CategoryIndustryObject;
import com.jobnow.models.CategoryIndustryResponse;
import com.jobnow.models.EmploymentObject;
import com.jobnow.models.ExperienceObject;
import com.jobnow.models.JobObject;
import com.jobnow.models.LevelObject;
...
public class PostAJobsActivity extends AppCompatActivity implements OnMapReadyCallback, View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            ...
            re_post=getIntent().getIntExtra("re_post",0);
            SetData(jobObject);
        ...
        btnSetDone.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (CheckOK()) {
                    ...
                    if (jobObject != null) {
                        if(re_post!=0) {
                            PostJobRequest request = new PostJobRequest(jobObject.id, jobTitle, "jobnow", 1, "", locationObject.id, categoryIndustryObject.id, Float.parseFloat(jobSalaryFrom), Float.parseFloat(jobSalaryTo), 2, ckShowSalary.isChecked() == false ? 0 : 1, levelObject.id, Float.parseFloat(latitude + ""), Float.parseFloat(longitude + ""), jobResponsibilities, jobRequiment, startDateTimeInterview, endDateTimeInterview, 1 , "", skillList, experienceObject.id + "", userModel.id, userModel.apiToken, employmentObject.id, Integer.parseInt(edtWorkinghour.getText().toString()));
                            EditPostAJobAsyntask postAJobAsyntask = new EditPostAJobAsyntask(PostAJobsActivity.this, request);
                            postAJobAsyntask.execute();

                            RePostAJobAsyntask rePostAJobAsyntask= new RePostAJobAsyntask(PostAJobsActivity.this,new RepostJobRequest(jobObject.id,userModel.id,userModel.apiToken));
                            rePostAJobAsyntask.execute();
                        }else {
    ...
    */
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        if (isGPSEnabled) {
            ...
            if (location != null) {
                ...
                if (jobObject != null) {
                    addMarkerChoice(new LatLng(Double.parseDouble(jobObject.Latitude), Double.parseDouble(jobObject.Longitude)));
                } else {
    ...
    // AsynTask
    class RePostAJobAsyntask extends AsyncTask<RepostJobRequest, Void, PostJobResponse> {
        ProgressDialog dialog;
        String sessionId = "";
        RepostJobRequest request;
        Context ct;

        public RePostAJobAsyntask(Context ct, RepostJobRequest request) {
            this.ct = ct;
            this.request = request;
        }

        @Override
        protected PostJobResponse doInBackground(RepostJobRequest... params) {
            try {
                JobController controller = new JobController();
                return controller.extendJob(request);
            } catch (Exception ex) {
                return null;
            }
        }
    ...
    class EditPostAJobAsyntask extends AsyncTask<PostJobRequest, Void, String> {
        ...
        protected void onPostExecute(String code) {
            try {
                if(re_post!=0){
                    Toast.makeText(PostAJobsActivity.this, "Congratulations! Job Re-Posting is created successfully", Toast.LENGTH_SHORT).show();
                }else {
                    Toast.makeText(PostAJobsActivity.this, "Job status updated successfully", Toast.LENGTH_SHORT).show();
                }
                Intent returnIntent = new Intent();
                returnIntent.putExtra("results", 150293);
                setResult(Activity.RESULT_OK, returnIntent);
